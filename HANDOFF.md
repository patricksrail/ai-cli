...
Restoration validation: 302 CLI tests and 20 website tests pass, including a real CLI/SDK Vercel retry regression against simulated HTTP. Typecheck and production builds pass. A fresh copy of the standalone package installed with ordinary `bun install`, built, and ran its preferred feed under Node; its generated lockfile contains no bricks dependency. See the fork README for the explicit request flow and file ownership.

Before restoration, validation passed: 294 CLI tests, 20 website tests and 8 library tests; production CLI/web builds, TypeScript and formatting pass. Lint retains 25 existing warnings and no errors. The rebuilt installed CLI returned `CLI_SHARED_OK` via Google Gemini 2.5 Flash Lite; the library SDK returned `LIBRARY_SHARED_OK` through OpenRouter with gateway-only BYOK auth. Failure recovery itself was verified with deterministic subprocess HTTP fixtures, avoiding manufactured live quota failures.

Patrick requested reversal of the unapproved dependency. The CLI is self-contained again: preferences and aliases are in `src/fork/model-preferences.{json,ts}`, generation policy in `generation.ts`, and the attempt engine in `fallback.ts`. Gateway authentication remains in `lib/gateway.ts`. Commands pass routing options explicitly. Bricks is separate and evolving. See README for `--preferred`, `--fallbacks`, `--no-fallback`, provider overrides and structured failure output.
...
## Upstream 0.5.2 and search opt-in (2026-09-21)

...
## Follow-up: search opt-in after quota diagnosis

Search is now off by default in CLI and bricks. With the same Google key, Gemini 3.8 returned 429 when search was enabled and 200 without search; subsequent requests also encountered temporary 503 high demand. This matches Google’s published free-tier search restriction and does not establish a faulty key. Both rebuilt projects passed the same independently checked news question on Gemini 2.5 Flash-Lite with explicit search enabled and fallback disabled. Bricks’ new `ai/examples/search-test.mjs` is a reusable test.

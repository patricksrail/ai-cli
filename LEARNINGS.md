...
## 2026-09-23: Wan `auto` can produce unsupported output dimensions

Verified on macOS with the existing Wan 2.2 5B image-to-video job: a 1024×768 source led Fal to choose automatic output dimensions 1088×800, then reject them after inference with HTTP 422. The [Fal schema](https://fal.ai/models/fal-ai/wan/v2.2-5b/image-to-video/api) documents `auto` but also lists only 16:9, 9:16 and 1:1 as explicit ratios; it documents center cropping when input and output ratios differ. A search for the exact worker error found no public matching issue, so prevalence is unknown. Other Fal video endpoints publish different ratio sets; do not generalize Wan's list.

Patrick chose to preserve the source by padding. Before this endpoint's submission, the CLI now chooses the nearest supported ratio, uses [Sharp edge extension](https://sharp.pixelplumbing.com/api-resize/#extend) to keep all source pixels, and sends the chosen ratio explicitly. Local tests verified exact source-pixel preservation and the CLI-to-SDK-to-Fal request body; the original image was visually inspected. This has not yet been verified by a new paid Wan inference. When a caller supplies a ratio, leave the image alone; Fal may crop. Reject unsupported explicit ratios before POST.

The first Bun production build bundled Sharp's JavaScript but could not locate its native darwin-arm64 binary, causing even `ai --version` to crash. Mark `sharp` external in `bun build`; the declared runtime dependency then resolves its native package normally. Always smoke-test the installed built command after adding a native dependency.

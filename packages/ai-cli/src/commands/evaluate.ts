/** Upstream SDK evaluation stays on an explicitly selected Vercel account.
 * No Cloudflare adapter is configured for Jev; never silently bypass BYOK. */
...
export function registerEvaluateCommand(program: Command) {
  const command = program
    ...
    .option("--gateway <name>", "Evaluation requires explicit vercel routing")
    .option(
      "--boolean <id=question>",
  ...
  addTimeoutOption(command, 30_000).action(
    async (_: undefined, options: EvaluateOptions) =>
      withGateway(options.gateway, async () => {
        ...
        if (resolveGatewayBackend() !== "vercel")
          throw new Error(
            "Evaluation requires Vercel: use --gateway vercel (and AI_GATEWAY_API_KEY). No Cloudflare evaluation adapter is configured."
          );

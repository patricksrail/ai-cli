...
// The default fork route must never spend against a different gateway silently.
test("evaluation requires deliberate Vercel routing", async () => {
  ...
  expect(blocked.stderr).toContain("--gateway vercel");
  expect(blocked.requests).toHaveLength(0);
  const allowed = await run(
    ["--gateway", "vercel", ...graphicArgs],
    "Ticket",
    { answers: mixedAnswers },
    { AI_CLI_GATEWAY: "cloudflare" }
  );

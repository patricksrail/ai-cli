...
import { classifyFailure } from "./fallback.js";
import { fallbackCandidates } from "./generation.js";
    ...
    const proc = Bun.spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          AI_GATEWAY_API_KEY: "fixture-vercel",
...
test("Vercel generation retains SDK retries without switching to Cloudflare", async () => {
  const result = await runCLI([
    "text",
    "--gateway",
    "vercel",
  ...
  expect(
    result.requests.every(
      (request) =>
        request.url.includes("ai-gateway.vercel.sh") &&
        request.url.endsWith("/language-model")
    )
  ).toBe(true);
  expect(JSON.parse(result.stdout).results[0].model).toBe("openai/gpt-5.6-sol");
});

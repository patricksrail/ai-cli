...
Fal video generation is asynchronous. ai-cli submits the request, polls its status, fetches the result, and downloads the video; do not call Fal's status URLs manually. Provider errors include Fal's `detail` or validation fields instead of an empty `Error:` line.

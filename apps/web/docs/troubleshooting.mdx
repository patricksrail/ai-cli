...
Cloudflare mode requires an authenticated gateway and an AI Gateway Run token. A missing token produces:

```
Error: CLOUDFLARE_AI_GATEWAY_TOKEN or CLOUDFLARE_API_TOKEN is required when AI_CLI_GATEWAY=cloudflare
```
...
`CLOUDFLARE_API_TOKEN` is a fallback when `CLOUDFLARE_AI_GATEWAY_TOKEN` is unset. Either token must grant **AI Gateway Run**.

...
With `AI_CLI_GATEWAY=cloudflare`, the first path segment selects a provider-native Cloudflare route:
...
Models use [Vercel AI Gateway](https://vercel.com/docs/ai-gateway) by default. Cloudflare mode sends supported models through [Cloudflare's provider-native routes](https://developers.cloudflare.com/ai-gateway/usage/providers/).

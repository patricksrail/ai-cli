...
The first two commands select the same underlying Gemini model through different providers. `google/...` uses the Google AI Studio key stored in Cloudflare; `openrouter/google/...` uses the stored OpenRouter key and asks OpenRouter to route the model.
...
The `models` command and short-name expansion still use Vercel's public model catalog. Use a full provider-prefixed ID when Cloudflare or a provider account exposes a different set of models.

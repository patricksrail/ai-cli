...
With `AI_CLI_GATEWAY=cloudflare`, `openai/...` and `google/...` use their provider-native Cloudflare routes. Prefix nested provider model IDs explicitly when needed:
...
Other creator IDs route through OpenRouter for text and Replicate for image/video while preserving the complete ID. Speech and transcription currently require an OpenAI or Google model.
